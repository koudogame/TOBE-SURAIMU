...
void Star::draw()
{
	...
	Sprite::getInstance()->reserveDraw(overlay_texture_, position_, trim, 1.0F, 14.0F, Vector2(1.0F, 1.0F), -( XMConvertToRadians( angle_[0] )- XM_PIDIV2), Vector2(kStarInformation[(static_cast<int>(size_) - kStarMin) / kStarDifference].z / 2.0F, kStarInformation[(static_cast<int>(size_) - kStarMin) / kStarDifference].z / 2.0F), Sprite::getInstance()->chengeMode());
	Sprite::getInstance()->reserveDraw( texture_ , position_ , trim , 1.0F , 14.5F , Vector2( 1.0F , 1.0F ) , -(XMConvertToRadians( angle_[ 0 ] )-  XM_PIDIV2 ) , Vector2( kStarInformation[ ( static_cast< int >( size_ ) - kStarMin ) / kStarDifference ].z / 2.0F , kStarInformation[ ( static_cast< int >( size_ ) - kStarMin ) / kStarDifference ].z / 2.0F ) );
}
